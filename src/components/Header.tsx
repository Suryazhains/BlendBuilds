...
// (Using the same image as a placeholder to prevent import errors)
import heroBg1 from '../assets/BackgorundCover.png';
import heroBg2 from '../assets/BackgorundCover.png'; 
import heroBg3 from '../assets/BackgorundCover.png'; 
...
const Home: React.FC = () => {
  ...
  // Background Slider Effect (Right to Left)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-black text-white relative font-sans overflow-x-hidden">
      ...
      {/* Background pattern */}
      <div className="absolute inset-0 opacity-20 pointer-events-none z-10" style={{ backgroundImage: 'radial-gradient(circle at center, #1a1a1a 1px, transparent 1px)', backgroundSize: '24px 24px' }}></div>

      <Header />

      {/* Hero Section */}
      <main className="relative flex flex-col items-start justify-end min-h-screen px-6 md:px-16 lg:px-24 pb-20 lg:pb-32 overflow-hidden border-b-2 border-[#1a1a1a]">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden bg-black">
          {backgroundImages.map((img, idx) => (
            <div
              key={idx}
              className="absolute inset-0 transition-transform duration-[1500ms] ease-in-out bg-cover bg-center"
              style={{ 
                backgroundImage: `url(${img})`,
                // Right to left slide mathematical logic
